...
def logger(path):
    def __logger(old_function):
        def new_function(*args, **kwargs):
            ...
            log_entry = f"{call_time} - Функция: {old_function.__name__}, Аргументы: {args}, {kwargs}, Результат: {result}\n"
...
def print_contacts(contacts, title="Контакты"):
    ...
    return f"Выведено {len(contacts) - 1} контактов"


@logger('phonebook.log')
def main():
    # Удаляем старый лог, если есть
    if os.path.exists('phonebook.log'):
        os.remove('phonebook.log')
    ...
    try:
        ...
        return f"Обработка завершена успешно. Создан файл {output_file}"
